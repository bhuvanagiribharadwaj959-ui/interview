...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  // Sync external props to refs for access inside animation loop
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (glContext.canvas instanceof Node) {
        container.appendChild(glContext.canvas);
      } else {
         // fallback or ignore if OffscreenCanvas in DOM context (unlikely here)
      ...
      const update = (t: number) => {
        ...
        // Handle AI speaking - drive orb from TTS audio output
        if (isSpeakingRef.current && externalAnalyserRef.current) {
          const extData = new Uint8Array(externalAnalyserRef.current.frequencyBinCount);
          externalAnalyserRef.current.getByteFrequencyData(extData);
          let extSum = 0;
          for (let i = 0; i < extData.length; i++) {
            const v = extData[i] / 255;
            extSum += v * v;
          }
          voiceLevel = Math.min(Math.sqrt(extSum / extData.length) * 3.0, 1.0);

          const voiceRotationSpeed = baseRotationSpeed + (voiceLevel * maxRotationSpeed * 2.5);
          if (voiceLevel > 0.03) {
            currentRot += dt * voiceRotationSpeed;
          }
          program.uniforms.hover.value = Math.min(voiceLevel * 2.5, 1.0);
          ...
          // Handle user voice input from microphone
      ...
      return () => {
        ...
        if (container && glContext && glContext.canvas) {
          try {
            if (container.contains(glContext.canvas as HTMLElement)) {
              container.removeChild(glContext.canvas as HTMLElement);
            }
          } catch (error) {
