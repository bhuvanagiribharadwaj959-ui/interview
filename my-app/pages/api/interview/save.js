...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      await setDoc(sessionRef, updatedData, { merge: true }).catch(() => {});
        ...
        for (let i = 0; i < transcript.length - 1; i++) {
          if ((transcript[i].role === 'assistant' || transcript[i].role === 'ai') && transcript[i+1].role === 'user') {
            ...
            await setDoc(evalRef, {
              ...
              score: Math.round((readinessScore || 70) / 10),
      ...
      console.warn('Firestore session save notice:', fsErr?.message || fsErr);
    }

    return res.status(200).json({ success: true, interviewId: finalSessionId });
  } catch (error) {
    console.error('Error saving session:', error);
    return res.status(200).json({ success: true, interviewId: interviewId || 'session_saved' });
  }
}
